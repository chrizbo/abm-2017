...
turtles-own [ adopted? threshold ] ;; add property of adoption
...
to setup
  ...
  if network = "random" [
    ;; cration of agents depending on slider
    nw:generate-random turtles links num-agents .1 [
      ;; adoption property needs to be init
      set adopted? false

      ;; set random threshold required for adoption
      set threshold random 100

      ;; separate the turtles spacially
...
to go
  if count turtles with [ not adopted? ] = 0
  [
    stop
  ]

  ;; ask turtles to adopt (or not adopt) randomly
...
to adopt
  ...
  if breed = influentials [
    ...
    if count total-neighbors > 0 [
      if not adopted? and
        random-float 1.0 < ( social-influence * ( count neighbors-adopted / count total-neighbors ) )
      [
        set adopted? true
        set color pink
      ]
  ...
  if breed = regulars [
    ...
    if not adopted? and
      random-float 1.0 < ( social-influence * neighbor-influence ) [
      set adopted? true
      set color pink
    ]
  ]
